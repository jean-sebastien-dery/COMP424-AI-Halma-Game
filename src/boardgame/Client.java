...
public class Client implements Runnable {
	...
	public static void main(String[] args) {
		if (args.length > 3) {
			...
			Client client;
			try {
				client = new Client(p, args.length > 1 ? args[1] : DEFAULT_SERVER, args.length > 2 ? Integer.parseInt(args[2]) : DEFAULT_PORT);
			} catch (Exception e) {
				printUsage();
				return;
			}
	...
	protected void processMessage(String msg) {
		if (msg.startsWith("GAMEOVER")) {
			...
			if (board.getTurn() != playerID || board.getWinner() != Board.NOBODY)
			...
			try {
				...
				System.err.println("Ingnoring unparseable move from server" + ": " + msg);
				return;
			}
			try {
				board.move(m);
	...
	protected void playMove() {
		...
		try {
			...
			System.err.println("ABORTING: Exception in " + player.getClass().getName() + ".choseMove()");
			e.printStackTrace();
			gameOver = true;
			return;
		}
